...
const {Tests,Questions,Suppliers,Reviews} = cds.entities('DevChallengeService');
module.exports = class DevChallengeService extends cds.ApplicationService {
        ...
        this.on('assignQuestionsToTest','Tests', async (req) => {
            ...
            .where('test_testId =', null );
             for (let question of questions) {
                ...
                .set({ test_testId: ID }) // Assign to the provided testId
                .where({ questionId: question.questionId })
                console.log(await SELECT. from('testmanagement.Questions'))
                // const ID= question.questionId;
                // await UPDATE(Questions,ID).with({test_testId:testId})
             }
             await UPDATE('testmanagement.Tests')
             .set({ assignedquestion: true }) // Assign to the provided testId
             .where({ testId: ID })
            ...
            console.log(`Assigned ${questionsCount} questions to the test with ID ${ID}`)
        });
    ...
    this.on('addReview', 'Tests', async req => {
        const ID=req.params[0].testId;
        const reviewText  = req.data.text; 
        const review={
            text: reviewText ,
            test_testId: ID
        }
        await INSERT (review).into (Reviews) ;
         console.log(await SELECT.from(Reviews)) ;
    });
...
// const cds = require('@sap/cds');
// const {Tests,Questions} = cds.entities('DevChallengeService');
// module.exports = class DevChallengeService extends cds.ApplicationService {
//     async init() {
//         this.on('assignQuestionsToTest','Tests', async (req) => {
//             const   questionsCount = req.data.questionsCount;
